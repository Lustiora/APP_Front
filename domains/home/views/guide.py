    ...
    return ft.Row(
        ...
        controls=[
            ...
            ft.Container(
                ...
                content=dogdog.basic_text(text, size=18, weight="bold", max_lines=3)
            )
        ]
    )

def what_guide(page: ft.Page, content):
    if content == "/what_bowel_score":
        content_column = [
            guide_row("bowel/01_poop.png", "1. 매우 딱딱하고 마름"),
            ft.Divider(),
            guide_row("bowel/02_poop.png", "2. 견고하나 딱딱하지 않고 잘 휘어짐"),
            ft.Divider(),
            guide_row("bowel/03_poop.png", "3. 표면이 촉촉하고 집어들 때 형태를 유지하나 바닥에 잔여물이 남음"),
            ft.Divider(),
            guide_row("bowel/04_poop.png", "4. 질척거림, 집어들 때 형태를 잃고 바닥에 잔여물이 남음"),
            ft.Divider(),
            guide_row("bowel/05_poop.png", "5. 매우 질척거리나 형태를 유지, 집어들 때 형태를 잃고 바닥에 잔여물이 남음"),
            ft.Divider(),
            guide_row("bowel/06_poop.png", "6. 물기가 몹시 많으나 질감은 있음"),
            ft.Divider(),
            guide_row("bowel/07_poop.png", "7. 완전한 액체 상태"),
        ]
    else:
        content_column = [
            dogdog.basic_text(
                spans=[
                    ft.TextSpan("강아지BCS(Body Condition Score, 신체충실지수)", style=dogdog.TextStyle(height=1)),
            ...
            dogdog.basic_text(
                spans=[
                    ft.TextSpan("[마름] 1~3단계\n", style=dogdog.TextStyle(height=1)),
